...
if not args.en_valid:  # When reading from en-10k and not from en-valid-10k
    ...
    train_stories, dictionary, labels = read_babi(path_babi_base, to_read_train, args.babi_tasks,
                                                  only_relevant=args.only_relevant)
    train_stories = vectorize_babi(train_stories, dictionary, args.batch_size_stories, device)
...
rrn = RRN(args.hidden_dim_rrn, args.message_dim_rrn, dict_size, args.f_dims, args.o_dims, device, g_layers=1,
          edge_attribute_dim=args.hidden_dim_lstm, single_output=True).to(device)
...
optimizer = torch.optim.Adam(chain(lstm.parameters(), rrn.parameters(), mlp.parameters()), args.learning_rate,
